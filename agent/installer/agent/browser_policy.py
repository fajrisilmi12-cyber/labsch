...
def apply_browser_policy(
    ...
    success = 0
    blocked_list = list(dict.fromkeys(str(s).strip() for s in blocked_sites if str(s).strip()))
    allowed_list = list(dict.fromkeys(str(s).strip() for s in allowed_sites if str(s).strip()))

    # A wildcard is written once at index 1. Any explicit domains follow it.
    specific_blocks = [site for site in blocked_list if site != "*"]

    for browser_name, policy_root in BROWSER_POLICIES.items():
        try:
            block_key = f"{policy_root}\\URLBlocklist"
            allow_key = f"{policy_root}\\URLAllowlist"
            _reg_delete_key(block_key)
            _reg_delete_key(allow_key)

            writes = [_reg_add(block_key, "1", "*")]
            writes.extend(
                _reg_add(block_key, str(i), site)
                for i, site in enumerate(specific_blocks, start=2)
            )
            writes.extend(
                _reg_add(allow_key, str(i), site)
            ...
            if not all(writes):
                log_fn(f"[browser_policy] registry write failed for {browser_name}")
                continue
