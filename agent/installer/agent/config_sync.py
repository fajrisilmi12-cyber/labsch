...
class AgentClient:
    ...
    def confirm_command(self, command: str, result: str = "success", reason: str = None) -> bool:
        """POST /api/command/{client_id}/confirm — v0.3.7+.

        Call after executing (or failing) a remote command.
        - result="success": server clears pending_command
        - result="failed":  server keeps pending_command for retry
        Best-effort, never raises.
        """
        from urllib.parse import quote
        body = {"command": command, "result": result}
        if reason:
            body["reason"] = reason
        resp = self._request("POST", f"/api/command/{quote(self.client_id)}/confirm", body)
    ...
    def get_pending_downloads(self, version="0.4.0-test1") -> Optional[list]:
        ...
        try:
            result = self._request(
                "GET",
                f"/api/downloads/pending?client_id={quote(self.client_id)}&protocol=1&version={quote(version)}",
                timeout=20,
            )
            return result.get("pending_downloads", []) if result is not None else None
        except Exception:
            return None

    def report_download_result(self, task_id, download_state=None,
                               execution_state=None, bytes_count: Optional[int] = None,
                               sha256: Optional[str] = None, error: Optional[str] = None) -> bool:
        """POST /api/downloads/report — durable download/execution outcome.

        Best-effort, never raises. Returns True when the server accepted it.
        """
        if isinstance(task_id, dict):
            r = task_id
            task_id, download_state, execution_state = r['task_id'], r['download_state'], r['execution_state']
            bytes_count, sha256, error = r.get('bytes'), r.get('sha256'), r.get('error')
        body = {
            "client_id": self.client_id,
            "task_id": task_id,
        ...
        if bytes_count is not None:
            body["bytes"] = bytes_count
        if sha256:
            body["sha256"] = sha256
        if error:
            body["error"] = str(error)[:500]
        try:
            result = self._request("POST", "/api/downloads/report", body, timeout=20)
            return result is not None and result.get("ok", False)
        except Exception:
            return False
